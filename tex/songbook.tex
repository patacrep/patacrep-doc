...
\item \command{\$ make book.pdf}~: la compilation de tous les carnets
  ...
  de \file{book.pdf}, vous pouvez mettre~:
\end{itemize}
...
\paragraph{Exécution}
Placez-vous dans votre répertoire \directory{songbook/}.
\begin{itemize}
\item Copiez les deux fichiers \file{make.bat} et \file{make.jar}
\item Exécutez le \file{make.jar}
\end{itemize}
...
\item \Key{Make Lilypond}~: générer les partitions Lilypond du
  dossier \directory{lilypond/}.
\item cliquer sur une version particulière du songbook pour le
  compiler.
\end{itemize}

\begin{nota}
  Les fichiers \file{make.bat} et \file{make.jar} sont disponibles
...
\file{header} et définir l'option paper-height de façon à ce que la
